...
def _paste_back(temp_frame, crop_frame, affine_matrix): 
	...
	inverse_mask_frame = cv2.erode(inverse_mask_frame, np.ones((2, 2), np.uint8)) 
	inverse_mask_border = inverse_mask_frame * inverse_crop_frame 
	inverse_mask_area = np.sum(inverse_mask_frame) / 3 
	inverse_mask_edge = int(np.sqrt(inverse_mask_area)) // 20 
	inverse_mask_radius = inverse_mask_edge * 2 
	inverse_mask_center = cv2.erode(inverse_mask_frame, np.ones((inverse_mask_radius, inverse_mask_radius), np.uint8)) 
